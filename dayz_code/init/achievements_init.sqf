...
if (isDedicated) then {
	...
	"achievement" addPublicVariableEventHandler {[_this select 1] spawn achievementServer};
};
...
if (!isDedicated) then {
	...
	"achievementClientPV" addPublicVariableEventHandler {[_this select 1] spawn achievementClientMsg};
